...
import { CHECKOUT_PATH } from '@/shared/routing/routes'
...
import { ListingDetailLocation } from '@/domains/marketplace/listings/presentation/marketplace/detail/ListingDetailLocation'
...
export function ProductDetailClient({
  ...
  useEffect(() => {
    ...
    setSelectedVariantId((current) => (current && variants.some((variant) => variant.id === current) ? current : null))
  }, [variants])
            ...
            <CardContent className='space-y-3 p-4'>
              <div className='space-y-1'>
                <div className='text-sm font-semibold'>
                  {variants.length === 1 ? 'Variante seleccionada' : 'Elegí una variante'}
                </div>
                <div className='text-xs text-muted-foreground'>
                  {variants.length === 1
                    ? 'Se selecciona automáticamente porque es la única disponible.'
                    : 'Las variantes disponibles dependen del stock actual.'}
                </div>
              </div>
